...
- `wiki-tools-*` — full mode (15 commands) with Git support, ~3.3MB
...
wiki-tools search "pattern" <PATH> --regex     # Regex search
wiki-tools backlinks <PAGE> <PATH>             # Who links to this page?
wiki-tools orphans <PATH>                      # Detect orphan documents
wiki-tools health <PATH>                       # Full health check (configurable weights)
wiki-tools trace <PAGE> <PATH>                 # Trace dependency chain
wiki-tools fix <PATH> --apply                  # Auto-fix broken links
wiki-tools fix <PATH> --apply --interactive    # Fix with per-item confirmation
wiki-tools index <PATH> --pretty               # Generate JSON index

# New in v1.1.0
wiki-tools rename <OLD> <NEW> <PATH> --dry-run # Rename doc + update all links
wiki-tools tags <PATH> --sort count            # List tags by usage
wiki-tools stats <PATH>                        # Wiki statistics overview
```
...
All query commands (`list`, `search`, `backlinks`, `orphans`, `health`, `trace`, `fix`, `index`, `rename`, `tags`, `stats`) work with both the Go binary and the native Claude Code skill. The `raw/` directory is excluded from `list` by default (use `--include-raw` to show it).

### Health Check Configuration

Customize health check weights in SCHEMA.md:
...
checks:
  - name: "frontmatter 完整性"
    command: "grep -rL 'tags:' entities/ concepts/ --include='*.md'"
    description: "检查哪些文档缺少 tags 字段"
    weight: 2
```
```
