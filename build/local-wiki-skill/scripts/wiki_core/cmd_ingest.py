...
def cmd_refresh(args: argparse.Namespace) -> None:
    """Scan normalized/ (and legacy raw Markdown) against wiki page sources."""
    path = expand(args.path or ".")
    require_wiki(path)

    normalized_dir = path / "normalized"
    raw_dir = path / "raw"
    if not normalized_dir.is_dir() and not raw_dir.is_dir():
        print(f"❌ normalized/ 和 raw/ 目录不存在: {path}")
        sys.exit(1)

    apply = getattr(args, "apply", False)

    # Collect platform-normalized Markdown first, plus legacy raw Markdown.
    source_files = []
    normalized_stems = set()
    if normalized_dir.is_dir():
        normalized_files = list(normalized_dir.glob("*.md"))
        normalized_stems = {f.stem for f in normalized_files}
        source_files.extend(f"normalized/{f.name}" for f in normalized_files)
    if raw_dir.is_dir():
        source_files.extend(f"raw/{f.name}" for f in raw_dir.glob("*.md") if f.stem not in normalized_stems)
    source_files = sorted(source_files)
    source_set = set(source_files)

    # Collect wiki pages (exclude source material and queries/)
    docs = collect_documents(path)
    wiki_docs = [d for d in docs if d["category"] not in ("raw", "normalized", "queries")]

    # Build set of referenced source files from frontmatter
    referenced_sources: set[str] = set()
    # Map: source_file -> [page that references it]
    ...
    for doc in wiki_docs:
        ...
        for src in sources:
            ...
            referenced_sources.add(src_norm)
            source_referrers.setdefault(src_norm, []).append(doc["file"])

    # New files: source Markdown not referenced by any wiki page
    new_files = sorted(source_set - referenced_sources)

    # Stale refs: referenced but file no longer exists on disk
    stale_refs = sorted(referenced_sources - source_set)
    stale_detail = {ref: source_referrers.get(ref, []) for ref in stale_refs}
    ...
    details = [f"New source files: {len(new_files)}", f"Stale references: {len(stale_refs)}"]
    if new_files:
    ...
    if args.format == "json":
        ...
        output = {
            ...
            "summary": {
                ...
                "processed": len(source_set & referenced_sources),
                "new": len(new_files),
        ...
        if new_files:
            ...
            output["agent_instruction"] = (
                "发现新增标准化资料。Agent 必须对每个新文件执行知识提取："
                "读取 normalized/ Markdown → 提取实体/概念/关系 → 创建 wiki 页面（含 frontmatter + wikilinks）→ 更新 log.md。"
    ...
    # Table output
    if not has_changes:
        print(f"✅ 刷新检查完成 — 无变更（{len(source_files)} 个资料均已处理）")
        return
    ...
    if new_files:
        print(f"📥 新增标准化资料（需要知识提取）: {len(new_files)} 个")
        for f in new_files:
